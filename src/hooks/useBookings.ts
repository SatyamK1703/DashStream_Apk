...
export const useMyBookings = (filters?: {
  ...
  return usePaginatedApi((params) => bookingService.getMyBookings({ ...filters, ...params }), {
...
export const useCreateBooking = () => {
  return useApi((bookingData: CreateBookingRequest) => bookingService.createBooking(bookingData), {
    showErrorAlert: true,
  });
};
...
export const useBookingDetails = (bookingId: string | null) => {
  ...
  const api = useApi(() => bookingService.getBookingById(bookingId!), {
    showErrorAlert: false,
    onSuccess: (data) => {
      if (__DEV__) {
      ...
      else if (data?.data?.booking) booking = data.data.booking;
      else if (data?.data && typeof data.data === 'object') booking = data.data;
      else if (typeof data === 'object') booking = data;

      if (booking) setProcessedData({ booking });
      else console.error('Could not extract booking data from API response');
    },
  });

  useEffect(() => {
    if (bookingId) api.execute();
  }, [bookingId]);

  return { ...api, data: processedData || api.data };
};

// Hook for canceling a booking
export const useCancelBooking = () => {
  return useApi(
    (bookingId: string, reason?: string) => bookingService.cancelBooking(bookingId, reason),
    {
      showErrorAlert: true,
    }
  );
...
export const useBookingTracking = (bookingId: string | null) => {
  ...
  const bookingDetailsApi = useApi(() => bookingService.getBookingById(bookingId!), {
    showErrorAlert: false,
    executeOnMount: false,
  });

  const api = useApi(() => bookingService.getBookingTracking(bookingId!), {
    showErrorAlert: false,
    onSuccess: (data) => {
