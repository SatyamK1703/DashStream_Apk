...
const ProfessionalCard = React.memo(({ 
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => {
        if (item.id) {
          console.log('Navigating to professional details:', item.id);
          navigation.navigate('AdminProfessionalDetails', { professionalId: item.id });
        } else {
...
const AdminProfessionalsScreen = () => {
  ...
  const handleLoadMore = useCallback(() => {
    if (!loading && pagination.hasMore) {
      loadMore({ page: pagination.page + 1 });
    }
  }, [loading, pagination.hasMore, pagination.page, loadMore]);
  ...
  const renderContent = () => {
    if (loading && !filteredProfessionals.length) {
      return (
    ...
    if (error && !filteredProfessionals.length) {
      return (
        <View style={styles.loadingContainer}>
...
// --- Styles ---
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      ) : error && !filteredProfessionals.length ? (
        <View style={styles.loadingContainer}>
          <Ionicons name="alert-circle-outline" size={64} color="#EF4444" />
          <Text style={styles.loadingText}>Failed to load professionals</Text>
          <TouchableOpacity 
